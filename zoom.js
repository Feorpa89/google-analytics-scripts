...
var can_gesture = false;

jQuery(window).bind("gesturestart",function(e){
  can_gesture = true;
});

var currentZoom = detectZoom.zoom().toFixed(2);

jQuery(window).bind("gestureend",function(e){
  if(can_gesture == true) {
    setTimeout(function() {
      dataLayer.push({
        'event': 'ga-event',
        'category': 'mobile',
        'action': 'user zoomed',
        'label': 'Zoomed using: pinch to scale ' + detectZoom.zoom().toFixed(2),
...
jQuery(window).bind("touchstart",function(e){
  can_touch = true;
});
...
jQuery(window).bind("touchend",function(e){
  if(can_touch == true) {
    setTimeout(function() {
      if(currentZoom != detectZoom.zoom().toFixed(2)) {
        dataLayer.push({
          'event': 'ga-event',
          'category': 'mobile',
          'action': 'user zoomed',
          'label': 'Zoomed using: double tap to scale ' + detectZoom.zoom().toFixed(2),
          'not_interactive': true
        });
        currentZoom = detectZoom.zoom().toFixed(2);
      }
    }, 250);
  }
  can_touch = false;
